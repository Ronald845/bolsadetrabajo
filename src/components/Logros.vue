...
export default {
  ...
  async mounted() {
    console.log('🔄 Logros mounted, cargando datos...')
  ...
  methods: {
    ...
    async cargarLogros() {
      if (!this.aspiranteId) {
        console.log('⚠️ No hay aspiranteId, no se pueden cargar logros')
        return
      }
      
      try {
        this.loading = true
        console.log('🏅 Cargando logros para aspirante:', this.aspiranteId)
        
        const response = await aspiranteService.obtenerLogros()
        console.log('📋 Todos los logros:', response)
        
        // Filtrar solo los logros del aspirante actual
        this.logros = response.filter(logro => logro.idAspirante === this.aspiranteId)
        console.log('✅ Logros del aspirante:', this.logros)
    ...
    async eliminarLogro() {
      try {
        ...
        console.log('🗑️ Eliminando logro:', this.logroAEliminar.idLogro)
        ...
        console.error('❌ Error completo guardando logro:', error)
  ...
  animation: slideInRight 0.6s ease-out;
    ...
    async guardarLogro() {
      ...
      try {
        ...
        console.log('💾 Guardando logro:', logroData)
        console.log('🔄 Editando:', this.editando)
        
        if (this.editando) {
          logroData.idLogro = this.form.idLogro
          console.log('📝 Actualizando logro existente con ID:', this.form.idLogro)
          const response = await aspiranteService.actualizarLogro(logroData)
          console.log('✅ Respuesta actualización:', response)
          this.showMessage('Logro actualizado exitosamente', 'success')
        } else {
          console.log('🆕 Creando nuevo logro')
          const response = await aspiranteService.crearLogro(logroData)
          console.log('✅ Respuesta creación:', response)
          this.showMessage('Logro agregado exitosamente', 'success')
        }
        
        await this.cargarLog